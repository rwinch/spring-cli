...
class PomUpdateActionHandlerTest {
	...
	void injectMavenDependency(@TempDir(cleanup = CleanupMode.ON_SUCCESS) Path workingDir) {
		this.contextRunner.withUserConfiguration(MockUserConfig.class).run((context) -> {
			...
			CommandRunner commandRunner = new CommandRunner.Builder(context)
					...
					.installCommandGroup("pom-update")
					.executeCommand("pom/update")
					.withArguments("name", "myname")
					.withArguments("version", "1.0.0.RELEASE")
					.withArguments("description", "mydescription")
					.build();
			commandRunner.run();

			Path pomPath = workingDir.resolve("pom.xml");

			assertThat(pomPath).content().contains("myname");
			assertThat(pomPath).content().contains("1.0.0.RELEASE");
			assertThat(pomPath).content().contains("mydescription");
